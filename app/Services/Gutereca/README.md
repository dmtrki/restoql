...
Gutereca aims to provide an easy way to integrate the Gutenberg editor with your Laravel projects. It takes the Gutenberg editor and adds all the communication and data it needs function in a Laravel environment. A demo can be found at [demo.laraberg.io](http://demo.laraberg.io/). If you would like to see an example of how to implement Gutereca you can take a look at the code from the demo  [over here](https://github.com/VanOns/laraberg-demo).
...
```bash
php artisan vendor:publish --provider="VanOns\Gutereca\GuterecaServiceProvider"
```

In order to store the data for the Gutenberg editor, Gutereca needs to run a database migration:
...
When updating Gutereca you have to publish the vendor files again by running this command:
```bash
php artisan vendor:publish --provider="VanOns\Gutereca\GuterecaServiceProvider" --tag="public" --force
```
...
```js
let content = Gutereca.getContent()
```
...
```js
Gutereca.setContent('content')
```

### Configuration options

The `init()` function takes an optional configuration object which can be used to change Gutereca's behaviour in some ways.
```js
const options = {}
Gutereca.init('[id_here]', options)
```
...
|---------------------------|-------------|---------------------------------------------------------------------------------------------|
|**sidebar**                |Boolean      |Enables the Gutereca sidebar if `true`                                                       |
...
In order to add the editor content to a model Gutereca provides the 'Gutenbergable' trait.

```php
use VanOns\Gutereca\Models\Gutenbergable;
...
Keep in mind that in order to correctly display some of the Wordpress styling the Gutereca CSS has to be present on the page:
...
Gutereca.registerBlock('my-namespace/my-block', myBlock)
```
...
Gutereca.registerCategory(title, slug)
```

## Events

Gutereca implements Laravel events that you can use to implement your own listeners. The events contain a 'content' attribute that contains the relevant Content object. For information on how Laravel events work you can read the [Laravel documentation](https://laravel.com/docs/5.8/events).

- VanOns\Gutereca\Events\ContentCreated
- VanOns\Gutereca\Events\ContentRendered
- VanOns\Gutereca\Events\ContentUpdated

## Sidebar

Gutereca provides a way to put your form fields in a seperate sidebar in the Gutenberg editor. This way you can let the editor take the entire screen while still having a place for your form fields. This is done by putting the input element in a parent element with the `.laraberg-sidebar` class. This is currently supported for the following input types:
- Checkbox
- Radio
- Select
- Text
...
For the labels Gutereca first checks if there is a label assigned to the element and will try to use that. If there is no label assigned it will check if there is a placeholder and use that as the label.

To enable the sidebar set the `sidebar` option to true when initializing Gutereca:
```js
Gutereca.init('[id_here]', { sidebar: true })
```
...
```js
Gutereca.init('[id_here]', { height: '500px' })

Gutereca.init('[id_here]', { maxHeight: '500px' })

Gutereca.init('[id_here]', { minHeight: '500px' })
```
...
```js
Gutereca.init('[id_here]', { prefix: '/[prefix_here]' })
```
...
```js
Gutereca.init('[id_here]', { laravelFilemanager: { prefix: '/[lfm_prefix_here]' } })
```
...
callback function when initializing Gutereca. This callback functions will receive a `search`, `perPage` and `type` parameter. The callback should return an array of objects that contain a title and an URL or a promise that resolves to such an array.
...
Gutereca.init('content', { searchCb: customSearch })
```
...
If you want to contribute to Gutereca checkout the [CONTRIBUTING.md](https://github.com/VanOns/laraberg/blob/master/CONTRIBUTING.md)
